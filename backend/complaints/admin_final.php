...
if (!empty($c['tech_id'])) {
    $techNotifStmt = $db->prepare("SELECT id FROM notifications WHERE for_tech = ? AND complaint_id = ? AND text LIKE '%Completed%'");
    $techNotifStmt->execute([$c['tech_id'], $id]);
