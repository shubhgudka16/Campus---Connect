...
function getDbConnection(): PDO {
    ...
    if ($pdo === null) {
        ...
        try {
            ...
            // Secondary attempt with fallback host (switch between localhost and 127.0.0.1)
            try {
                $fallbackHost = ($host === 'localhost') ? '127.0.0.1' : 'localhost';
                $dsnFallback = "mysql:host={$fallbackHost};port={$port};dbname={$dbname};charset=utf8mb4";
                $pdo = new PDO($dsnFallback, $user, $pass, $options);
            } catch (PDOException $e2) {
                // Log the technical error details securely to PHP server log
                error_log("Database connection failure [Primary: " . $e1->getMessage() . "] [Fallback: " . $e2->getMessage() . "]");
                sendJson(
                    false,
                    'Database connection failed. Please make sure MySQL is running and the campus_connect database is imported.',
